...
from helpers.maps import (connect_to_database, fetch_values, fetch_lonames_for_ano,
                          current_dir, create_full_map, extract_full_mappings,
                          extract_machines_and_problems_of_area, create_time_availability_machines,
                          create_time_availability_lines, month_map, stat_headers, tables, barcharts)
...
def get_area_for_line(line):
    with connect_to_database() as conn:
        full_map = create_full_map(conn)
        area_line_map, _, _, _ = extract_full_mappings(full_map)
...
def get_area_for_machine(machine):
    with connect_to_database() as conn:
        full_map = create_full_map(conn)
        area_line_map, line_machine_map, _, _ = extract_full_mappings(full_map)
    for area, lines in area_line_map.items():
        for line in lines:
...
def get_occurrences_machines_problems(machines_and_problems, dataframe, required_area, month):
    container_widget = QWidget()
    widget_layout = QHBoxLayout(container_widget)
    widget_layout.setAlignment(Qt.AlignCenter)
    counts = {problem: {'count': 0, 'machines': set()} for _, problem in machines_and_problems}
    for machine, problem in machines_and_problems:
        counts[problem]['machines'].add(machine)
    for index, row in dataframe[dataframe["MONTH"] == month].iterrows():
        total_time = row["TOTAL TIME"]
        if total_time > 0:
            area = row["AREA"]
            if area == required_area:
                machine = row["MACHINE"]
                problem = row["PROBLEM"]
                if (machine, problem) in machines_and_problems:
                    counts[problem]['count'] += 1
    sorted_counts = {k: v for k, v in sorted(counts.items(), key=lambda item: item[1]['count'], reverse=True)}
    table_widget = QTableWidget(len(sorted_counts), 3)
    month_title = get_key(month_map, month)
    table_widget = TitledTableWidget(f"No of Occurrence in {required_area} - {month_title}", table_widget)
    table_widget.setHorizontalHeaderLabels(
        ["Machine", "Problem", f"No of Occurrence"])
    table_widget.setEditTriggers(QAbstractItemView.NoEditTriggers)
    ...
    occurrences = [v['count'] for v in sorted_counts.values()]
    machines = [', '.join(v['machines']) for v in sorted_counts.values()]
    for i, (machine_list, problem, occur) in enumerate(zip(machines, problems, occurrences)):
        item_machines = QTableWidgetItem(machine_list)
        item_machines.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
        item_machines.setTextAlignment(Qt.AlignCenter)
        table_widget.setItem(i, 0, item_machines)
        ...
        table_widget.setItem(i, 1, item_problem)
        item_occurrence = QTableWidgetItem(str(occur))
        item_occurrence.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
        item_occurrence.setTextAlignment(Qt.AlignCenter)
        table_widget.setItem(i, 2, item_occurrence)
    table_widget.resizeRowsToContents()
    table_widget.resizeColumnsToContents()
    table_widget.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
    table_widget.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
...
def get_bd_machines(month, dataframe, time_availability_machines, line_edits, required_header,
                    machines_and_problems, required_areas=None):
    container_widget = QWidget()
    widget_layout = QHBoxLayout(container_widget)
    widget_layout.setAlignment(Qt.AlignCenter)
    machines = set()
    ...
    if required_areas == ["OT CELL", "IT GRD"]:
        for area in required_areas:
            with connect_to_database() as conn:
                machines_and_problems = extract_machines_and_problems_of_area(conn, area)
                for machine, problem in machines_and_problems:
        ...
        sorted_machines = sorted(machines, key=lambda x: bd_percentages.get(x, 0), reverse=True)
    else:
        for machine, problem in machines_and_problems:
            df_machine = dataframe[(dataframe['MACHINE'] == machine) & (dataframe["MONTH"] == month)]
            machine_total_sum = df_machine["TOTAL TIME"].sum()
            ...
            if machine_avail_time * num_days != 0:
                ...
                bd_percentages[machine] = np.nan
            machines.add(machine)
        sorted_machines = sorted(machines, key=lambda x: bd_percentages[x], reverse=True)
    table_widget = QTableWidget(len(sorted_machines), 3)
    month_title = get_key(month_map, month)
    table_widget = TitledTableWidget(f"% of B/D in {month_title}", table_widget)
    table_widget.setHorizontalHeaderLabels(
        ["Machine", f"% of B/D", "Target in %"])
    table_widget.setEditTriggers(QAbstractItemView.NoEditTriggers)
    ...
    table_widget.setFixedWidth(800)
    table_widget.setStyleSheet('''
        QTableWidget {
            border: none;
        }
    ...
    machine_bd_percs = [bd_percentages[machine] for machine in sorted_machines]
    for i, (machine, machine_bd_perc, target) in enumerate(zip(sorted_machines, machine_bd_percs, targets)):
        ...
        table_widget.setItem(i, 0, item_machine)
    ...
    ax.barh(list(reversed(sorted_machines)), list(reversed(machine_bd_percs)))
    ax.set_title(f"{required_header} - {month_title}")
    if required_header == "SX Damper & FA":
    ...
    ax.set_yticklabels(list(reversed(sorted_machines)))
    if required_header == "SX Damper & FA":
    ...
    bar_chart_widget = FigureCanvas(fig)
    bar_chart_widget.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)
    bar_chart_widget.setFixedSize(1000, 600)
    widget_layout.addWidget(bar_chart_widget)
    return table_widget, bar_chart_widget, container_widget
...
def display_statistics(tree_widget_left, scroll_area):
    ...
    with connect_to_database() as conn:
        ...
        full_map = create_full_map(conn)
        area_line_map, line_machine_map, machine_problem_map, problem_caction_map = extract_full_mappings(full_map)
        ...
        machines_and_problems_shox = extract_machines_and_problems_of_area(conn, 'SHOX')
        machines_and_problems_fffa = extract_machines_and_problems_of_area(conn, 'FFFA')
        machines_and_problems_otcell = extract_machines_and_problems_of_area(conn, 'OT CELL')
        machines_and_problems_itgrd = extract_machines_and_problems_of_area(conn, 'IT GRD')
        conn.commit()
                ...
                if child_text in stat_headers:
                    ...
                    if (child_text == stat_headers[1] or child_text == stat_headers[2]
                        ...
                        table_widget, _, container_widget = run_function(current_month, child_text, df,
                                                                         ...
                                                                         machines_and_problems_shox,
                                                                         machines_and_problems_fffa,
                                                                         machines_and_problems_otcell,
                                                                         machines_and_problems_itgrd)
...
def run_function(month, stat_header, dataframe, line_edits, area_stats_header, lines1_stats_header,
                 ...
                 time_availability_machines, machines_and_problems_shox, machines_and_problems_fffa,
                 machines_and_problems_otcell, machines_and_problems_itgrd):
    ...
    if stat_header == stat_headers[0]:
        ...
        table_widget, container_widget = get_occurrences_machines_problems(machines_and_problems_shox, dataframe,
                                                                           "SHOX", month)
    elif stat_header == stat_headers[3]:
        table_widget, container_widget = get_occurrences_machines_problems(machines_and_problems_fffa, dataframe,
                                                                           "FFFA", month)
    elif stat_header == stat_headers[4]:
        table_widget, container_widget = get_occurrences_machines_problems(machines_and_problems_otcell, dataframe,
                                                                           "OT CELL", month)
    elif stat_header == stat_headers[5]:
        table_widget, container_widget = get_occurrences_machines_problems(machines_and_problems_itgrd, dataframe,
                                                                           "IT GRD", month)
