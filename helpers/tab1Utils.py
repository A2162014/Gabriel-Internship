...
def validate_and_check_item(table_widget, item):
    ...
    if item.text():
        if column_name == 'MONTH':
            ...
            for col in range(table_widget.columnCount()):
                if table_widget.horizontalHeaderItem(col).text().strip() == 'DATE':
                    ...
                    break
            if date_column_index is not None and item.text() != '':
        ...
        if column_name == 'DATE':
            ...
            if not item.text().isalpha() or not item.text().isupper():
                show_error_message("Invalid characters in INCHARGE column. Only capital alphabets are allowed.",
                                   table_widget)
                item.setText("")
    col = item.column()
    if col == 7:
...
def tab_changed(index, tab_widget, main_window):
    ...
    if not tab_widget.isTabEnabled(index):
        QMessageBox.information(main_window, "Tab Disabled",
                                '<span style="font-size: 12pt;">Please open an Excel file to access this tab.</span>')
    if index == 2 and table_edited:
        reply = QMessageBox()
        reply.warning(main_window, "Table Edited",
                      '<span style="font-size: 12pt;">The table has been edited. Please reopen the changed file to view updated statistics.</span>',
                      QMessageBox.Ok)
...
def open_table(right_line_edit, table_widget, tab_widget):
    if table_edited:
        reply = QMessageBox().question(
            None, 'Save Changes',
            '<span style="font-size: 12pt;">Do you want to save changes before opening a new file?</span>',
...
def new_table(right_line_edit, table_widget, tab_widget):
    global table_edited, df
    if table_edited:
        reply = QMessageBox.question(None, 'Save Table?',
                                     '<span style="font-size: 12pt;">Do you want to save the current table before clearing it?</span>',
...
def save_table(table_widget):
    ...
    if file_path:
        try:
            ...
            df.to_excel(file_path, index=False)
            QMessageBox.information(None, 'Saved Table', f'Table saved to {file_path}.')
        except Exception as e:
            show_error_message(f'Error saving table: {e}')
